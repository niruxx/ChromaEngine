...
void ThumbnailGenerator::setSoftwareRenderingEnabled(bool enabled)
{
...
void ThumbnailGenerator::startNext()
{
    ...
    // Parked off-screen so it never needs to be composited (or even
    // on-screen) at all - see the Windows ThumbnailGenerator's class
    // comment for the background on why an on-screen-but-hidden window was
    // tried first and didn't reliably stay invisible.
    m_captureWidget = new QWidget();
    m_captureWidget->setWindowFlags(Qt::FramelessWindowHint | Qt::Tool | Qt::WindowDoesNotAcceptFocus);
    ...
    // winId() is the X11 Window (XID) here - already a plain integer, no
    // native handle type to cast through the way Windows' HWND needs.
    const int64_t wid = static_cast<int64_t>(m_captureWidget->winId());
    const QString path = m_activePath;
    const bool softwareRendering = m_softwareRenderingEnabled;

    // mpv_create/mpv_initialize/loadfile all run off the main thread - only
    // the QWidget above (needed for its window ID) has to be created here.
    ...
    // stutter beyond the screenshot capture itself.
    QMetaObject::invokeMethod(
        m_workerContext,
        [this, wid, path, softwareRendering]() {
            ...
            // See MpvSurface.cpp's constructor comment: on a machine with no
            // working GL/EGL, the default "gpu" vo can fail to embed into
            // this capture widget's wid at all, falling back to a stray
            // top-level mpv window instead - "x11" needs no GPU context.
