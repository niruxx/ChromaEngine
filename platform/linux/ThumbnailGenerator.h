...
// decoded, downscaled, and written to disk - long enough (especially with
// the real wallpaper's own mpv instance also competing for the CPU) to
// visibly stall the UI if run on the main thread, since it shares this
// process's single Qt event loop. That work (plus the immediate re-read of
// the file) runs on a single persistent background thread instead - reused
// for every capture via queued invocation, rather than spun up and torn
// down per capture. Only the cheap mpv session setup/tear-down and Qt
// signal handling stay on the UI thread.
...
public:
    ...
    // See MpvSurface's constructor comment for why this exists - same
    // GPU-less rendering path, same reason (a foreign-window "wid" embed can
    // otherwise silently fail on a machine with no working GL/EGL). Takes
    // effect for files enqueued after the call.
...
private:
    ...
    bool m_softwareRenderingEnabled = false;
